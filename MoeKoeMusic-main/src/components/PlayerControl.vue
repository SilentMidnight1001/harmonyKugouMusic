...
const { playbackModes, currentPlaybackModeIndex, currentPlaybackMode, playedSongs, playHistory, resetShuffle, togglePlaybackMode, setPlaybackMode } = playbackMode;
...
const playSong = async (song) => {
    ...
    try {
        ...
        // 记录已播放歌曲（随机模式），用于不断从未播放歌曲中抽取新歌
        if (currentPlaybackModeIndex.value === 0 && song.hash) {
            playedSongs.value.add(song.hash);
            if (playHistory.value[playHistory.value.length - 1] !== song.hash) {
                playHistory.value.push(song.hash);
...
// 处理随机播放逻辑：从歌单中抽取「未播放过」的新歌，全部播完后重置再开始新一轮
const handleRandomPlayback = (direction, currentIndex) => {
    const queue = musicQueueStore.queue;
    if (queue.length === 0) return 0;

    // 上一首：从播放历史中回退
    if (direction === 'previous') {
        if (playHistory.value.length > 1) {
            playHistory.value.pop(); // 移除当前（最后）一首
            const prevHash = playHistory.value[playHistory.value.length - 1];
            const idx = queue.findIndex(s => s.hash === prevHash);
            if (idx !== -1) return idx;
        }
    ...
    // 下一首：筛选未播放过的歌曲
    let unplayed = queue.filter(s => !playedSongs.value.has(s.hash));

    // 全部已播放：清空记录，重新开始新一轮（并排除当前歌曲，避免紧接着重复）
    if (unplayed.length === 0) {
        playedSongs.value.clear();
        playHistory.value = [];
        unplayed = queue.filter(s => !playedSongs.value.has(s.hash));
        if (unplayed.length > 1 && currentIndex >= 0 && currentIndex < queue.length) {
            unplayed = unplayed.filter(s => s.hash !== queue[currentIndex].hash);
        }
    }

    const selected = unplayed.length > 0
        ? unplayed[Math.floor(Math.random() * unplayed.length)]
        : queue[Math.floor(Math.random() * queue.length)];

    // 标记为已播放，避免下一次又抽到同一首（playSong 里还会补记录历史）
    playedSongs.value.add(selected.hash);

    const idx = queue.findIndex(s => s.hash === selected.hash);
    return idx !== -1 ? idx : 0;
};
...
defineExpose({
    ...
    addLocalPlaylistToQueue: async (localSongs, append = false, startIndex = null) => {
        ...
        if (!append && queueSongs.length > 0) {
            ...
            // 指定了起始歌曲时，从该歌曲开始播放
            if (startIndex !== null && startIndex !== undefined && startIndex >= 0 && startIndex < queueSongs.length) {
                songIndex = startIndex;
                console.log('[PlayerControl] 从指定本地歌曲开始播放:', queueSongs[songIndex].name);
            } else if (currentPlaybackModeIndex.value == 0) {
                // 如果是随机播放模式，则随机选择一首歌曲
    ...
    getPlaylistAllSongs,
    addPlaylistToQueue: async (info, append = false, startIndex = null) => {
        ...
        if (songs && songs.length > 0 && !append) {
            ...
            // 指定了起始歌曲时，从该歌曲开始播放
            if (startIndex !== null && startIndex !== undefined && startIndex >= 0 && startIndex < songs.length) {
                songIndex = startIndex;
                console.log('[PlayerControl] 从指定歌曲开始播放:', songs[songIndex].name);
            } else if (currentPlaybackModeIndex.value == 0) {
                // 如果是随机播放模式，则随机选择一首歌曲
    ...
    addCloudPlaylistToQueue: async (songs, append = false, startIndex = null) => {
        ...
        if (queueSongs && queueSongs.length > 0 && !append) {
            ...
            if (startIndex !== null && startIndex !== undefined && startIndex >= 0 && startIndex < queueSongs.length) {
                songIndex = startIndex;
                console.log('[PlayerControl] 从指定云盘歌曲开始播放:', queueSongs[songIndex].name);
            } else if (currentPlaybackModeIndex.value == 0) {
                // 如果是随机播放模式，则随机选择一首歌曲
                songIndex = Math.floor(Math.random() * queueSongs.length);

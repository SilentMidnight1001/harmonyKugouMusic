...
// 播放歌曲
const playSong = async (hash, name, img, author) => {
    // 先加载完整歌单，再整单入队，从点击的歌曲开始播放，
    // 避免只把已加载的部分歌曲放进队列，导致随机/顺序播放反复循环同一批歌。
    await loadAllRemainingTracks();
    const index = tracks.value.findIndex(track => track.hash === hash);
    props.playerControl.addPlaylistToQueue(tracks.value, false, index >= 0 ? index : 0);
};
...
const addPlaylistToQueue = async (event, append = false) => {
    ...
    if (append) {
        // 追加模式：把当前列表追加到现有队列
        props.playerControl.addPlaylistToQueue(filteredTracks.value, true);
    } else {
        // 先加载完整歌单，再整单入队并播放，保证随机/顺序覆盖所有歌曲
        await loadAllRemainingTracks();
        props.playerControl.addPlaylistToQueue(tracks.value, false);
    }
};

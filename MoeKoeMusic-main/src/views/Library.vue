...
const playSong = (hash, name, img, author) => {
    // 点击单首歌曲时，把当前听歌历史作为播放队列，并从点击的歌曲开始播放，
    // 避免队列中只有这一首歌，导致各种播放模式下反复播放同一首歌。
    const index = listenHistory.value.findIndex(song => song.hash === hash);
    const info = listenHistory.value.map(song => ({
        hash: song.hash,
        name: song.name,
    ...
    props.playerControl.addPlaylistToQueue(info, false, index >= 0 ? index : 0);
};

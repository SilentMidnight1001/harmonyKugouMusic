...
const isOnline = ref((typeof window !== 'undefined' && window.electron?.platform === 'ohos') ? true : navigator.onLine);
...
const handleOffline = () => {
    if (typeof window !== 'undefined' && window.electron?.platform === 'ohos') return; // 鸿蒙上 navigator.onLine 误报为 false，实际网络可用

...
export function createWindow() {
    ...
    const useNativeTitleBar = process.platform === 'ohos' ? true : (savedConfig?.nativeTitleBar === 'on' ? true : false);
    ...
    mainWindow.on('close', (event) => {
        if (process.platform === 'ohos') {
            const savedConfig = store.get('settings');
            if (savedConfig?.minimizeToTray === 'off' || app.isQuitting) {
                // 设置关闭即退出，或应用正在退出：直接结束进程
                app.isQuitting = true;
                app.exit(0);
                return;
            }
...
const getIconPath = (iconName, subPath = '') => {
    const basePath = process.platform === 'ohos'
        ? path.join(__dirname, '../../icons') // HarmonyOS: 图标资源随 resfile 打包，相对 __dirname 定位
        : (isDev ? __dirname + '/../build/icons' : process.resourcesPath + '/icons');
    return path.join(basePath, subPath, iconName);
};
...
export function createTray(mainWindow, title = '') {
    ...
    } else if (process.platform === 'ohos') {
        trayIconName = 'OIP-C.png'
    } else {
        trayIconName = 'tray-icon.ico'
    }

    try {
        tray = new Tray(nativeImage.createFromPath(getIconPath(trayIconName)));
        tray.setToolTip('MoeKoe Music');
    } catch (error) {
        console.log('[createTray] 创建托盘失败:', error);
        tray = null;
        return null;
    }
...
export function startApiServer() {
    return new Promise((resolve, reject) => {
        let apiPath = '';
        if (process.platform === 'ohos') {
            // HarmonyOS 无独立可执行文件，改为在进程内启动 KuGouMusicApi（Express）
            process.env.PORT = '6521';
            process.env.HOST = '127.0.0.1';
            process.env.platform = 'lite';
            import('../api/server.js')
                .then((m) => (m.startService || m.default.startService)())
                .then(() => {
                    log.info('HarmonyOS: 内置 API 服务器已启动 @ http://127.0.0.1:6521');
                    resolve();
                })
                .catch((err) => {
                    log.error('HarmonyOS: 内置 API 服务器启动失败:', err);
                    reject(err);
                });
            return;
        }

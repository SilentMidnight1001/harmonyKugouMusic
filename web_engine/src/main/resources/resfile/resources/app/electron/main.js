...
if (settings?.preventAppSuspension === 'on' && process.platform !== 'ohos') {
...
ipcMain.on('window-control', (event, action) => {
    console.log('[window-control] received:', action);
    switch (action) {
        case 'close':
            if (process.platform === 'ohos') {
                if (store.get('settings')?.minimizeToTray === 'off' || app.isQuitting) {
            ...
            } else if (store.get('settings')?.minimizeToTray === 'off') {
                app.isQuitting = true;
                app.quit();
            } else {
                mainWindow.close();
        ...
        case 'minimize':
            if (process.platform === 'ohos') {
                // HarmonyOS 主窗口无 window.minimize()，改用 hideAbility() 收起到后台
                mainWindow.hide();
            } else {
                mainWindow.minimize();
            }
...
ipcMain.on('save-settings', (event, settings) => {
    ...
    if (['on', 'off'].includes(settings?.autoStart) && process.platform !== 'ohos') {
